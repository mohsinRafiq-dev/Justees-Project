...
const CategoriesManagement = () => {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        // Reset to square aspect for new images
        setCropAspect(1);
      };
  ...
  const applyCrop = async () => {
    ...
    try {
      ...
      const file = new File(
        [blob],
        selectedImage ? selectedImage.name : "cropped.jpg",
        { type: blob.type },
      );
      setSelectedImage(file);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(
          res.error || `Failed to ${editingId ? "update" : "add"} category`,
        );
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="p-8">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div
            className={`${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} rounded-2xl max-w-md w-full shadow-2xl p-4 sm:p-6 border my-8`}
          >
            <h3
            ...
            <div className="space-y-6">
              ...
              {/* Actions */}
              <div
                className={`flex justify-end gap-3 pt-4 border-t ${isDark ? "border-gray-700" : "border-gray-100"}`}
              >
                <button
                  onClick={closeModal}
                ...
                <div className="flex items-center gap-4 flex-1">
                  {c.image ? (
                    <img
                      src={c.image}
                      alt={c.name}
                      className={`w-12 h-12 sm:w-12 sm:h-12 rounded object-cover border flex-shrink-0 ${isDark ? "border-gray-700 bg-gray-900" : "border-gray-200 bg-gray-100"}`}
                    />
                  ) : (
                    <div
                      className={`w-12 h-12 sm:w-12 sm:h-12 rounded flex items-center justify-center text-xs flex-shrink-0 ${isDark ? "bg-gray-900 text-gray-500" : "bg-gray-200 text-gray-500"}`}
                    >
                      No Img
                    </div>
                  )}
