  ...
  const applyCrop = async () => {
    ...
    try {
      ...
      if (err.message.includes("CORS")) {
        toast.error(
          "Cannot crop this image — CORS policy blocks access. Try re-uploading the image.",
        );
      } else {
  ...
  const openModal = (slide = null) => {
    if (slide) {
      ...
      setFormData({
        ...
        type:
          slide.type ||
          (slide.url && slide.url.includes(".mp4") ? "video" : "image"),
        mediaUrl: slide.url || "",
        order: slide.order || 0,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(
          res.error || `Failed to ${editingId ? "update" : "add"} slide`,
        );
      }
    } catch (err) {
      console.error(err);
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="flex items-center justify-center w-full">
                  {previewUrl ? (
                    showCropper && formData.type === "image" ? (
                      <div
                          ...
                          <div className="absolute bottom-16 left-4 right-4 z-10">
                            <div className="text-xs text-white/80 bg-black/30 px-3 py-2 rounded text-center">
                              {cropAspect === undefined
                                ? "Free cropping - drag corners and edges to select any area"
                                : `Fixed ${cropAspect === 1 ? "square" : "aspect ratio"} - drag to position`}
                            </div>
                          </div>
                ...
                <div className="relative">
                  ...
                  <AnimatePresence>
                    {showCategoryDropdown && (
                      <div
                        className={`absolute z-50 w-full mt-1 border rounded-lg shadow-xl overflow-hidden max-h-48 overflow-y-auto ${
                          isDark
            ...
            {slides.map((s) => (
              ...
              >
                <div className="flex items-center gap-4 flex-1">
                  <div
                    className={`w-20 h-12 rounded overflow-hidden border flex-shrink-0 ${isDark ? "bg-gray-900 border-gray-700" : "bg-gray-100 border-gray-200"}`}
                  >
                    {s.type === "video" ? (
                      <video
                        src={s.url}
